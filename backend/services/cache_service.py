...
class CacheService:
    ...
    def _generate_key(self, user_id: int, portfolio_id: Optional[int] = None) -> str:
        """
        ...
        """
        key_data = f"portfolio_analysis_{user_id}"
        if portfolio_id:
            key_data += f"_{portfolio_id}"
        
        return hashlib.md5(key_data.encode()).hexdigest()
    
    def get(self, user_id: int, portfolio_id: Optional[int] = None) -> Optional[dict]:
        """
        ...
        if expires_at and datetime.utcnow() > expires_at:
            del self.cache[key]
            logger.info(f"Cache abgelaufen für User {user_id}")
            return None
        
        logger.info(f"Cache Hit für User {user_id}")
        return cached_item.get('data')
    
    def set(self, user_id: int, data: dict, portfolio_id: Optional[int] = None) -> None:
        """
        ...
        logger.info(f"Cache gespeichert für User {user_id}, läuft ab um {expires_at}")
    
    def invalidate(self, user_id: int, portfolio_id: Optional[int] = None) -> None:
        """
